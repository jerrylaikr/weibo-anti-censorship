...
from weibo_spider.parser import AlbumParser, IndexParser, PageParser, PhotoParser
...
class SpiderRev:
    ...
    魔改自原作者的Spider类
    魔改要素：
        去除所有flags，单纯作为一个api使用（？）
        去除魔改mongo writer之外的writers
        since_date和end_date不受config.json及user_id_list.txt影响
        不会更新user_id_list.txt中的since_date
        加入类方法用于override时间区间：set_time_range()
    """
    ...
    def __init__(self, config):
        ...
        self.since_date = str(since_date)  # 起始时间，即爬取发布日期从该值到结束时间的微博，形式为yyyy-mm-dd
    ...
    def set_time_range(self, since_date, end_date):
        """override每个用户的爬取时间区间"""
        self.since_date, self.end_date = since_date, end_date
        for user_config in self.user_config_list:
            user_config["since_date"] = since_date
            user_config["end_date"] = end_date
    ...
    def get_weibo_info(self):
        ...
        try:
            ...
            ).replace(tzinfo=pytz.timezone("PRC"))
            now = datetime.now().astimezone(pytz.timezone("PRC"))
            if since_date <= now:
                ...
                if (
                    ...
                    logger.info("即将进入全局等待时间，%d秒后程序继续执行" % wait_seconds)
                ...
                for page in tqdm(range(1, page_num + 1), desc="Progress"):
                    ...
                    if self.page_count >= self.global_wait[0][0]:
                        logger.info("即将进入全局等待时间，%d秒后程序继续执行" % self.global_wait[0][1])
    ...
    def initialize_info(self, user_config):
        ...
        self.weibo_id_list = []
        self.writers = [MongoWriterRev(self.mongo_config)]
    ...
    def start(self):
        ...
        try:
            if not self.user_config_list:
                logger.info("没有配置有效的user_id，请通过config.json或user_id_list.txt配置user_id")
                return
            user_count = 0
...
def _get_config(config_filename: str = None):
    """获取config.json数据(魔改版)"""
    if not config_filename:
        config_path = os.getcwd() + os.sep + "config.json"
    else:
        config_path = os.getcwd() + os.sep + config_filename
    try:
        ...
        logger.error(
            "config.json 格式不正确，请访问 " "https://github.com/dataabc/weiboSpider#2程序设置"
        )
        sys.exit()


def main_rev(since_date, end_date):
    """魔改main"""
    try:
        config = _get_config()
        config_util.validate_config(config)
        wb = SpiderRev(config)
        wb.set_time_range(since_date, end_date)
        logger.info(
            f"Running rev with since_date = {since_date}, end_date = {end_date}"
        )
        wb.start()  # 爬取微博信息
    except Exception as e:
        raise
...
class MongoWriterRev(MongoWriter):
    def write_weibo(self, weibos):
        """(回溯检测用)将爬取的微博信息写入MongoDB数据库"""
        weibo_list = []
        for w in weibos:
            w.user_id = self.user.id
            weibo_list.append(w.__dict__)
        self._info_to_mongodb("weibo_rev", weibo_list)
        logger.info("%d条微博写入MongoDB数据库(coll=weibo_rev)完毕", len(weibos))
